...
async function handlePhaseCall({
  ...
  try {
    const { data, error } = await supabase
      ...
      .select("*")
      .eq("request_key", http_Request_Key)
  ...
  try {
    ...
    if (error || !data) {
      ...
      return {response: new Response(
        JSON.stringify({ error: `handlePhaseCall/${phase}/http_request_mapping_warehouse: Failed to fetch mapping for: ${http_Request_Key}` }),
        {
          status: 404,
          headers: jsonHeaders,
        }
  ...
  const {response, updatedIDs} = await constructLLMCall({
          ...
          multiPhaseIDs,
          ef_log_id,
          assistant_key: wf_record.wf_assistant_name,
        });
...
let logTable;
  ...
  try {
        ...
        await logLLMRequest(supabase, user, logTable, {
          ...
          method: method,
          ef_log_id,
          assistant_key: logMeta.assistant_key,
        });
      } catch (logErr) {
